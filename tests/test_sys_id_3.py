...
Object assumed to have TWO regions of mass distribution through mass mapping
'''
...
# map half particles to mass 0 and half to mass 1
mapping = np.ones(sim_gt.num_particle)
mapping[:int(sim_gt.num_particle/2)] = 0
...
for i in range(h):
    ax.plot(traj[i][:,0], color=c[1], alpha=0.3)
    ax.plot(traj[i][:,1], color=c[3], alpha=0.3)
